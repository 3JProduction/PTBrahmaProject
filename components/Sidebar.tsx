...
export default function Sidebar({ role }: { role: string }) {
  ...
  const navItems = [
    ...
    { href: '/dashboard/issues', label: 'Laporan Kendala', icon: AlertCircle },
  ];
    ...
    <>
      ...
      `}>
        ...
        <div className="p-6 border-b border-gray-100 hidden md:block">
          <h1 className="text-2xl font-bold text-blue-700">PT BWAT</h1>
          <p className="text-sm text-gray-500 mt-0.5">Project Management</p>
        </div>
        ...
        <div className="p-4 border-t border-gray-200 mt-auto bg-gray-50">
          <div className="text-sm text-gray-500 mb-3 hidden md:block">
            Login sebagai:<br/>
          ...
          <form action={logout}>
            <button 
              type="submit" 
              className="w-full flex items-center gap-2 px-3 py-2.5 text-red-600 hover:bg-red-50 rounded-lg font-medium text-sm transition"
            >
              <LogOut size={18} />
              {/* Teks diubah menjadi Logout di bawah ini */}
              <span>Logout</span>
            </button>
          </form>
        </div>
